export default {
  // navbar
  mainPageText: "Басты бет",
  videoText: "Видео",
  tournamentsText: "Турнирлер",
  regulationsText: "Ережелер",
  coachesText: "Жаттықтырушылар",
  newsPageText: "Жаңалықтар",
  aboutUsPage: "Біз туралы",
  findPartnerPageText: "Серіктес табу",
  profileText: "Профиль",
  adminUsersPageText: "Пайдаланушылар",
  adminStatsPageText: "Статистика",
  informationAboutParticipant: "Қатысушы туралы ақпарат",
  tierText: "Деңгей",

  welcome: "Қош келдіңіз",
  registrationText: "Тіркелу",
  enterName: "Атыңызды енгізіңіз",
  enterSecondName: "Тегіңізді енгізіңіз",
  enterEmail: "Электронды поштаңызды енгізіңіз",
  enterPassword: "Құпиясөзді енгізіңіз",
  serviceText: "Қызмет түрі",
  ratingText: "Рейтинг",
  experienceText: "Тәжірибесі",
  notSpecifiedText: "Көрсетілмеген",
  cityText: "Қала",
  languageText: "Тіл",
  stadiumText: "Стадион",
  costText: "Бағасы",
  description: "Сипаттама",
  closeButton: "Жабу",
  firstNameText: "Аты",
  lastNameText: "Тегі",
  ageText: "Жасы",
  emailText: "Электрондық пошта",
  fullNameText: "Толық аты-жөні",
  personalDataText: "Жеке деректер",
  genderText: "Жыныс",
  phoneNumber: "Телефон нөмірі",
  professionalInformationText: "Кәсіби ақпарат",
  createButton: "Жасау",
  timePart: {
    label: "Уақыты",
    caption: "Жасалған және жаңартылған уақыты",
    createdAt: "Жасалған күні",
    updatedAt: "Жаңартылған күні",
  },
  profile: {
    partnerText: "Серіктес",
    tournamentText: "Турнир",
    categoryText: "Санат",
    statusText: "Статус",
    acceptButton: "Қабылдау",
    rejectButton: "Бас тарту",
  },

  registration: {
    button: "Тіркелу",
    accountText: "Аккаунтыңыз бар ма? Кіру",
    carousel: {
      mail: {
        title: "Пошта",
        text: `Электронды поштада символы болуы керек. Дұрыс және жұмыс істейтін электронды пошта мекенжайын енгізгеніңізге көз жеткізіңіз.`,
      },
      password: {
        title: "Құпиясөз",
        text: `Құпиясөз кемінде 6 таңбадан тұрып, арнайы символдарды қамтуы тиіс.`,
      },
    },
  },
  authorization: {
    text: "Сайтқа кіру",
    button: "Кіру",
    welcomeBack: "Қайта келдіңіз",
    accountText: "Аккаунтыңыз жоқ па? Тіркелу",
  },
  indexPage: {
    tournamentPage: {
      newsText: "Жаңалықтар",
      allNews: "Барлығы",
      interview: "Сұхбаттар",
      championships: "Чемпионаттар",
      analysis: "Талдау",
    },
    achievementsPage: {
      achievementText: "Біздің жетістіктер",
      tournamentsHeld: "Өткізілген турнирлер",
      usedBalls: "Қолданылған доптар",
      numberOfPlayers: "Ойыншылар саны",
      subscribers: "Жазылушылар",
    },
    parallaxPage: {
      block1: {
        text: "Неге бізді таңдайды?",
      },
      block2: {
        text: "Бірегей тәсіл",
        caption: "Әр қатысушыға тәлімгерлік және жеке назар",
      },
      block3: {
        text: "Өсу мен даму",
        caption: "Біз дағдылар мен тұлғалық дамуды әр кезеңде қолдаймыз.",
      },
      block4: {
        text: "Күшті қауымдастық",
        caption: "Сен жылы әрі қолдаушы қауымдастықтың бір бөлігі боласың.",
      },
      block5: {
        text: "Дағдыларды шыңдау",
        caption: "Нағыз турнирлер, челлендждер және өзіңді көрсету мүмкіндігі.",
      },
    },
    meetFounders: {
      mainText: "Негізін қалаушылармен танысыңыз",
      founder1: {
        fullName: "Батыр Ашим",
        role: "Фронтенд әзірлеуші",
      },
      founder2: {
        fullName: "Арыстанбек Әбуев",
        role: "Бэкенд әзірлеуші",
      },
      founder3: {
        fullName: "Ақжан Өмір",
        role: "Бэкенд әзірлеуші",
      },
    },
  },
  coachPage: {
    mainText: "Бапкерлер бағдарламасына қатысу үшін өтінім жіберіңіз.",
    mainTextWithAnotherColor: "Біз жақын арада сізбен байланысамыз.",
    captionText: `Өтінім беру, өз тәжірибеңізбен бөлісу және кәсіби бапкерлер командасының бір бөлігі болу үшін төмендегі форманы толтырыңыз.`,
    search: "Іздеу...",
    addFormText: "Форма қосу",
    clickToViewDetailedInformation:
      "Бапкер туралы толық ақпаратты көру үшін мұнда басыңыз",
    noData: "Бапкерлер туралы хабарландырулар жоқ",
    editCoach: {
      updateButton: "Жаңарту",
    },
    createCoach: {
      mainText: "Тренер құру",
      sendButton: "Жіберу",
    },
    detailedInformation: {
      mainText: "Толық ақпарат",
      captionTextOfPersonalData:
        "Толық аты-жөні, электрондық пошта, телефон нөмірі",
      captionTextOfProfessionalInformation: "Тәжірибе, қызмет, бағасы",
    },
  },
  findPartnerPage: {
    mainText: "Өзіңізге сай теннис серіктесін тауып, байланыс орнатыңыз.",
    noData: "Серіктес туралы хабарландыру жоқ",
    mainTextWithAnotherColor: "Барлығы бір жерде.",
    search: "Іздеу...",
    captionText:
      "Ойыншыларды деңгейі, орналасқан жері немесе қолжетімділігі бойынша іздеңіз — ойынды бүгіннен бастаңыз!",
    sendRequest: "Серіктес сұранысын қосу",
    detailedInformationAboutPartner:
      "Серіктес туралы толық ақпаратты көру үшін мұнда басыңыз",
    detailedInformation: {
      mainText: "Толық ақпарат",
      captionTextOfPersonalData:
        "Толық аты-жөні, электрондық пошта, телефон нөмірі",
      captionTextOfProfessionalInformation: "Тәжірибе, қызмет, бағасы",
    },
    postRequest: {
      button: "Жариялау",
    },
    editPartnerInformation: {
      edit: "Өңдеу",
    },
  },
  newsPage: {
    mainText:
      "Соңғы спорт жаңалықтары, сараптамалық пікірлер және ойыншылар туралы ақпараттар.",
    mainTextWithAnotherColor: "Барлығы бір жерде.",
    captionText: `Теннис жайлы өзекті жаңалықтармен, эксклюзивті сұхбаттармен және сахна сыртындағы материалдармен хабардар болыңыз.`,
    search: "Іздеу...",
    openAddNewsBlock: "Жаңалық қосу",
    editNews: "Өңдеу",
    deleteNews: "Жою",
    addNews: {
      mainText: "Жаңалық қосу",
      title: "Тақырыпты енгізіңіз",
      description: "Сипаттаманы енгізіңіз",
      uploadPhoto: "Фотосуретті жүктеңіз",
      closeButton: "Жабу",
      addNewsButton: "Қосу",
    },
  },
  tournamentPage: {
    mainPage: {
      participants: "Қатысушылар",
      exploreButton: "Зерттеу",
    },
    mainText: {
      text1: "ЖАСАУШЫЛАР",
      text2: "ОРТАЛЫҒЫ",
    },
    captionText: {
      text1: "Бұл қауымдастық керемет",
      text2: "Бәріне мақтанышпен қараймыз",
      text3: "кәсіби мамандар күтіп отыр.",
    },
    editPage: {
      mainText: "Ақпаратты өңдеу",
      startDateText: "Басталу күні",
      endDateText: "Аяқталу күні",
      startTimeText: "Басталу уақыты",
      categoryText: "Санат",
      maxParticipantsText: "Қатысушылардың максималды саны",
      locationText: "Өтетін орны",
      minLevelText: "Ең төменгі деңгей",
      maxLevelText: "Ең жоғарғы деңгей",
      closeButton: "Жабу",
      updateButton: "Жаңарту",
    },
    exploreTournament: {
      additionalInformation: "Қосымша ақпарат",
      participants: "Қатысушы(лар)",
      registerToTheTournament: "Турнирге тіркелу",
      searchPartner: "Серіктес іздеу",
      levelDoesNotMatch: "Сіздің деңгейіңіз турнир талаптарына сәйкес келмейді",
      generateBracket: "Турнир кестесін жасау",
      calculateThePoints: "Ұпайларды есептеу",
      participantName: "Қатысушының аты",
    },
    searchPartnerPage: {
      searchUser: "Пайдаланушыны іздеу",
      usersText: "Пайдаланушы(-лар)",
      closeButton: "Жабу",
      inviteButton: "Шақыру",
    },

    bracketPage: {
      finalText: "Финал",
    },

    userStats: {
      statsText: "Пайдаланушы статистикасы",
      tierMatchText: "Матч деңгейі",
    },

    internationalTournamentsText: "Халықаралық турнирлер",
    fromText: "Басталуы",
    toText: "Аяқталуы",
    noData: "Қосымша турнирлер жоқ...",
  },
  editPage: {
    tabs: {
      generalInfo: "Жалпы ақпарат",
      myTournaments: "Менің турнирлерім",
      settings: "Баптаулар",
    },
    editInformationButton: "Ақпаратты өңдеу",
    logoutButton: "Шығу",
    dateOfBirth: "Туған күні",
    noData: "Деректер жоқ",
    selectLanguage: "Тілді таңдаңыз",
    selectButton: "Таңдау",
    invitationsText: "Шақыру(-лар)",
    editBlock: {
      editText: "Өңдеу",
      closeButton: "Жабу",
      updateButton: "Жаңарту",
    },
  },

  ratingPage: {
    leaderboardText: "Көшбасшылар тақтасы",
    pointsText: "Ұпайлар",
  },

  detailedInformationAboutParticipant: {
    tournamentName: "Турнир атауы",
    tournamentTier: "Турнир деңгейі",
    opponentName: "Қарсыластың аты",
    winnerText: "Жеңімпаз",
    tournamentContinueText: "Турнир жалғасуда",
    victories: "Жеңістер",
    defeat: "Жеңіліс",
    distribution: "Тарату",
    matchHistory: "Матчтар тарихы",
  },

  regulationsPage: {
    titleText: "Турнир ережелері мен регламенті",
    eligibility: "Қатысу талаптары",
    registration: "Тіркелу",
    codeOfConduct: "Мінез-құлық кодексі",
    tournamentFormat: "Турнир форматы",
    matchRules: "Матч ережелері",
    playerSafety: "Ойыншы қауіпсіздігі",
    prizeDistribution: "Сыйлықтарды бөлу",
    tournamentSchedule: "Турнир кестесі",
    description: {
      eligibilityDesc:
        "Барлық қатысушылар турнирге қатысу үшін кемінде 18 жаста болуы керек.",
      registrationDesc:
        "Қатысушылар турнир басталғанға дейін ресми жүйе арқылы тіркелуі керек.",
      codeOfConductDesc:
        "Барлық қатысушылар мінез-құлық кодексін сақтауы және жақсы спорттық мінез көрсетуі тиіс.",
      tournamentFormatDesc:
        "Турнир айналмалы форматта өткізіліп, кейінгі кезеңдерде шығару жүйесімен жалғасады.",
      matchRulesDesc:
        "Барлық матчтар ресми теннис ережелеріне сай, стандартты ұпай және құралдармен өтеді.",
      playerSafetyDesc:
        "Ойыншылар кез келген жарақатты бірден ресми тұлғаларға хабарлауы керек және денсаулық себептерімен жарыстан шығарылуы мүмкін.",
      prizeDistributionDesc:
        "Сыйлықтар әр санат бойынша алғашқы үш орынға беріледі.",
      tournamentSchedule:
        "Турнирдің толық кестесі онлайн қолжетімді болады және қажет болған жағдайда жаңартылып отырады.",
    },
  },
  aboutUsPageText: {
    title: `Қатысушыларды тіркеу және түрлі жарыс форматтарын қолдау арқылы теннис турнирлерін ұйымдастыру жүйесін әзірлеу`,
    projectTeamText: "Жоба тобы",
    founders: `Ақжан Өмір (Backend-әзірлеуші), Арыстанбек Абиев (Backend-әзірлеуші), Батыр Ашим (Frontend-әзірлеуші)`,
    aim: `Бұл дипломдық жоба — теннис турнирлерін ұйымдастыруға арналған заманауи жүйені әзірлеуге бағытталған.
        Жоба түрлі жарыс форматтарын, ойыншыларды тіркеуді, серіктестер мен жаттықтырушыларды іздеуді және турнирді басқару функцияларын қамтиды.`,
    systemOverview: "Жүйе сипаттамасы",
    platformDescription:
      "Теннис турнирлерін басқаруға арналған кешенді платформа",
    functionalities: `Жүйеде үш рөл бар: қарапайым қолданушы, ұйымдастырушы және әкімші.
        Қарапайым қолданушылар турнирлерге тіркеліп, серіктестер мен жаттықтырушыларды іздей алады, жаттықтырушы болуға өтініш бере алады және жаңалықтар оқи алады.
        Ұйымдастырушылар турнирлер құрып, оларды басқарады, нәтижелерді енгізеді, жаңалықтар жариялайды және өтініштерді мақұлдай немесе қабылдамай тастай алады.
        Әкімшілер барлық пайдаланушылар мен өтініштерді толық бақылайды.`,
    stackText: "Қолданылған технологиялар",
    stacks: "Quasar, Vue.js, Tailwind, Spring Boot, Docker, PostgreSQL",
    frontendText: `Фронтенд Vue 3 және Quasar Framework көмегімен әзірленген, Tailwind CSS арқылы стильдендірілген.
        Бэкенд Java Spring Boot негізінде жазылған.
        Жүйе Docker арқылы орналастырылып, Netlify және Cloudflare R2 арқылы хостингке шығарылған.
        Қосымша Cordova арқылы мобильді құрылғыларды қолдайды және Vitest көмегімен тестіленген.`,
    userRoleAndFutures: "Қолданушы рөлдері мен мүмкіндіктері",
    functionalityBreakdown: "Функцияларға бөлінуі",
    regularUser: {
      title: "Қарапайым қолданушы",
      features: `Турнирге тіркелу, серіктес/жаттықтырушы іздеу, жаттықтырушы болуға өтініш беру, теннис жаңалықтарын оқу.`,
    },
    organizer: {
      title: "Ұйымдастырушылар",
      features: `Турнир құру және басқару, тор құру, нәтижелерді енгізу, турнирді аяқтау, ұпай беру, жаңалықтармен жұмыс істеу, өтініштерді мақұлдау/қабылдамау.`,
    },
    admin: {
      title: "Әкімші",
      features: `Пайдаланушы контентін модерациялау, өтініштерді қарау және бекіту, пайдаланушылар мен деректердің тұтастығын басқару.`,
    },
    rankingSystem: "Рейтинг жүйесі",
    playerPerformanceTracking: "Ойыншы нәтижелерін қадағалау",
    systemFeature: `Барлық турнирге қатысушылар автоматты түрде рейтинг жүйесіне енгізіледі.
        Ұпайлар олардың нәтижелеріне байланысты беріледі.
        Турнир аяқталғаннан кейін рейтинг жаңартылады.`,
  },
};
